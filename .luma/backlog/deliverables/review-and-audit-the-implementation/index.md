...
- **Several bugs were found by running the tool rather than by its tests** — empty frontmatter failing to parse, a stamp written as a quoted string, an outcome given a deliverable's status. Each passed a suite that asserted on the wrong thing. That pattern is unlikely to have stopped.
