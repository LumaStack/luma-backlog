...
created knowingly in the wrong shape — this is a delivery and work items are all we have, which is what BACK-0069 exists to fix; recorded on the record itself so a reader does not mistake it for a piece of work
the order is forced by BACK-0037: the window for migration being a one-repo problem closes on first use elsewhere, not on a date — so the breaking changes finish before anybody else starts, or every later shape change becomes a distributed migration on data we cannot see
