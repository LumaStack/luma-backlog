...
evidence rather than argument: BACK-0077's default list of preparation steps, written before this idea, splits two and two along exactly this seam — coordinate and deliverables on one side, outcomes set and outcomes measurable on the other
...
that is the sequential-or-parallel question workflow-status.md already worked out, and the two answers cost very different amounts — sequential is 'more rungs in order, configuration, no code'; parallel 'does not fit, a record holds one workflow_status', so it cannot be a status at all and has to travel alongside the position like blocked (BACK-0065)
this project is evidence and it points at NO: BACK-0074's outcomes were written while its scope and constraints were still moving, and the gap found at its gate today — two worktrees of one corpus — is a PREPARATION fact discovered after the outcomes existed
...
PROPOSED: a gate or a field that says when we think the outcomes are DEFINED — which is this record's definition pipeline needing an exit condition it can actually check, rather than a judgement somebody announces
backlog-move already says outcome refinement is 'the only thing in this rung with a natural stopping point', so the stopping point exists and nothing records that it was reached. a field would be the record of it
the four candidate values are NOT one axis: workable and bulletproof measure how strong the definition is; flexible and rigid measure how much it is expected to change. one is quality, the other is a commitment about the future, and a record can be bulletproof AND flexible or workable AND rigid
so this may be two fields rather than one — which is the same shape as status-versus-rank and owner-versus-worker, both of which were found by trying to make one field carry two axes. see BACK-0036, where the same proposal lands as a job for stage
