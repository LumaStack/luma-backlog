# Journal — Whether stage is used correctly or removed
...
and it is not the axis for whether a retired outcome held — BACK-0071 carries that, since stage answers whether to load a record and not what happened to it
closing sets stage to stable, and under this reading that is exactly right rather than merely conventional: a closed work item is still current context that nobody will update

## ▶ 2026-09-06

stage is set once by the scaffold and never touched again — every record here is draft, including five decisions promoted by hand and every outcome since proven
