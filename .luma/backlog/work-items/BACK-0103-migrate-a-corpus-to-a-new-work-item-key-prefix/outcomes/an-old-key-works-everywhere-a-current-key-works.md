...
verify_by: "For a migrated record, run each key-accepting command with the old key and the new one and compare: show, set, transition, rank, close, journal -w, and -w on outcome/task/exploration new. Results match, and output shows the current key."
