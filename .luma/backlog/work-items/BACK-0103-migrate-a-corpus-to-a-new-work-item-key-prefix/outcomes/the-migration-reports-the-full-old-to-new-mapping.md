...
verify_by: "Run the migration and capture the output: one line per migrated record, old key and new key, machine-readable, count equal to the number migrated. Over a constructed corpus with collisions, every skipped record appears with the key that blocked it and the record that holds it. A run that skipped nothing says so rather than printing an empty section."
