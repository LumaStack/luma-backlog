...
found by using it — BACK-0016 was closed with 'set workflow_status=closed' and ended up in the terminal status with no outcomes, which is exactly what close refuses and BACK-0032 ranks worst of three
...
it also matters for tasks, which have no close verb at all: today a task ends via set, so whatever answer this takes has to leave that path working or give tasks a close of their own (BACK-0064)
...
and it leaves the task case: tasks have no close verb, so today they end through set. refusing a terminal status on a task would break the only path they have — which is why BACK-0064 (a task cannot record why it ended) has to land first or move with it
