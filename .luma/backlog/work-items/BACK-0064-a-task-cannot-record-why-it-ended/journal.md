...
so backlog-move 0.33.0 carries a row that says 'every task that ran, succeeded — warned' and marks it unbuilt against this record. the promise is visible rather than quietly absent, which is the BACK-0075 lesson applied forward
...
added to the record itself rather than left in a journal line: warn on a completed close if any task was not successful. it is the first stated requirement this record has, where before it argued from the model alone
