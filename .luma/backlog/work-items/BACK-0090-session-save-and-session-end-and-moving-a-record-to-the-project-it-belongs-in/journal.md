...
and it interacts with keys: BACK-0090 here and BACK-0090 there are different records with the same handle, so a cross-project reference has to carry the project as well as the key — which is the same shortfall as BACK-0013's collision problem seen from outside one corpus
THIRD INSTANCE in a row: BACK-0091 is an idea about a project-wizard bundle, which belongs to luma-catalog and was captured here. three consecutive ideas have now been filed in the wrong project's corpus, which is the strongest evidence this record has
FOURTH INSTANCE: BACK-0093 is an idea about luma-foreman, a third project — not this one, not luma-catalog. four consecutive ideas now filed in the wrong corpus, spanning three destinations
CHECKED rather than assumed, 2026-09-10: luma-foreman and luma-catalog are both checked out at /Users/warden/Workspace/code/lumastack/, one directory from here. both have .luma/ and NEITHER has .luma/backlog/ — so there is no corpus in either to file a record into
which means moving a record between projects has a prerequisite nobody had named: the destination needs a corpus, and creating one is a decision about whether that project keeps a backlog at all. that is BACK-0070, and it sits in front of this rather than beside it
and the agent had been asserting 'there is nowhere else to put it' across four captures without once looking. it was true, and it was a guess — which is the kind that stops getting checked because it keeps coming out right
