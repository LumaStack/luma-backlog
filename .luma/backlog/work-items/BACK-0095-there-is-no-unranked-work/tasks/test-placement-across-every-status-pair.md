...
description: One test per ordered pair, so a status quietly exempted from the rule fails rather than passing. Assert order, never specific position values --- BACK-0096 may change what Between allocates, and a test pinned to numbers would break on a scheme change that broke nothing. Advances outcome 3.
