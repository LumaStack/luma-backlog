...
not blocking the start, but this is the last cheap moment to fix an outcome — after this it is Redefine, which is where goalposts move (BACK-0032)
the move wrote workflow_status and rank and nothing else — internal/app/status.go:21 does exactly that and no more, so backlog-move's guarantee table is unkept in more rows than BACK-0075 names
