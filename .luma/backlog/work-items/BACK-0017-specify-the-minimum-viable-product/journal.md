...
attempt state ships as a single field on the outcome, with the attempt ledger deferred to BACK-0019 — the field must be shaped so a list can subsume it later without a contract break
...
rank ships with --before/--after/--top/--bottom only; --at deferred to BACK-0021 because it names a slot in an unspecified list, and additions are non-breaking while removals are not
