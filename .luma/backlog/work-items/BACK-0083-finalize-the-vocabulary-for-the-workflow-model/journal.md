...
STRUCTURAL QUESTION LEFT OPEN, and it outranks the word: exploration and inquiry may be one concept at two scales. BACK-0079 established that exploration lives inside a work item OR as a work item when the investigation is the work, which is what kind: inquiry means. if those collapse, the name falls out of that rather than being chosen ahead of it
BRAINSTORM for what to call workflow_status, 2026-09-09, four generation strategies. from the ladder metaphor: rung, step, landing, tier, notch. from what the field answers: position, standing, progress, point, place. borrowed from bounded-ordered-state domains: station, waypoint, leg, berth, lifecycle. from what the organization owes the record: commitment, readiness, footing, bearing, custody
the maintainer ruled out the metaphor family — rung is a term the agent introduced ad hoc and has been used inconsistently in many places, so it is not a foundation to build on. and law/medicine/aviation/publishing analogies were rejected as unrelated: the inspiration should come from backlog and work-management systems
PATH NOT TAKEN, and worth not re-proposing: modelling the ladder as TWO axes — what has been promised (nothing / to understand / to deliver) crossed with how far along it is (waiting / active / done), which produces the seven rungs as a 3x3 grid with dead cells
rejected for a hard reason rather than taste: NO major system models it as two axes, and integration requires one. every tool maps a single ordered field onto a small category set, so a second axis has nothing to map to. re-open only if integration stops being a requirement
and the naming landed on bare status — state claims more than the field holds, since a record's state is the whole record while its status is where it stands in a process. that is BACK-0089 now, with the survey of twenty tools in its body
...
stage cannot be considered for any job here until BACK-0036 settles — it is also a FIELD carrying draft, provisional, stable, and that is an internal collision rather than an external one
...
EVIDENCE from BACK-0081: three checks now derive rungs positionally — the pile, the shaping rung, the started rung — because nothing in the ladder names them. an organization inserting a step silently moves all three, which is the 'do gates survive more steps' question answered in running code: no, and not because gates are unmodeled but because RUNGS have no identity beyond position
