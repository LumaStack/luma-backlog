...
evidence:
  - at: "2026-09-09T23:48:06Z"
    by: agent:claude-opus-5/luma-backlog
    what: All four checks run against a scratch corpus in /tmp/vt2 with the built binary. (1) transition BACK-0001 closed exits 2 naming close and listing the four dispositions; set BACK-0001 workflow_status=closed exits 2 naming transition — so neither route reaches the terminal status. TestTransitionToTerminalIsRefusedAndNamesClose asserts the record is unchanged afterwards. (2) close BACK-0001 canceled succeeds on a work item with no outcomes and show --json then reads workflow_status closed — cancelling is ungated by design. (3) close BACK-0001 completed on the same record is refused with "has no outcomes, so there is nothing that says it was completed. Declare what done means, or close with a different disposition" — it names what is missing rather than a flag to type. (4) git log over internal/app/close.go and internal/cli/close.go shows zero commits since this work began at 016eeca.
---
