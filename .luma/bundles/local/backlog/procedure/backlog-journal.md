...
The journal is **the work item's memory**. A session loses its memory when it ends; this is what survives. Its test:
...
`.luma/backlog/work-items/<slug>/journal.md`, for the work item the work belongs to. If the work does not belong to one, that is usually a sign a work item is missing — not that the note belongs somewhere else.
