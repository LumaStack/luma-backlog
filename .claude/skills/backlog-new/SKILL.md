...
- **Two different status fields.** `workflow_status` is where the work is — `idea`, `preparing`, `actionable`, `todo`, `in_progress`, `closed`. `lifecycle_status` is how much to trust the record — `draft`, `provisional`, `stable`, `archived`. They are unrelated.
...
- **Never delete.** Archive by setting `lifecycle_status: archived`.
