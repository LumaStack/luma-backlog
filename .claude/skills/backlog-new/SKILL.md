...
- **Two different status fields.** `workflow_status` is where the work is — `idea`, `preparing`, `actionable`, `todo`, `in_progress`, `closed`. `stage` is how much to trust the record — `draft`, `provisional`, `stable`, `archived`. They are unrelated.
