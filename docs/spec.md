...
The format's `stage` — `draft`, `provisional`, `stable`, `archived` — is unchanged and stays orthogonal by the format's own design. Formation says *how ready the work is*; `stage` says *how much the record can be relied upon*. A deliverable can be `actionable` and `draft` if the plan is complete but nobody trusts it yet.
...
Creating a successor therefore writes exactly one file and touches nothing shared, which is the same property that makes membership work (§3.2). The format supports this directly, defining supersession as a relationship rather than a `stage` value.
