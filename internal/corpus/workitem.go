...
func matchesWorkItem(dirName, filter string) bool {
	...
	if m := dirPattern.FindStringSubmatch(dirName); m != nil {
		// Compared as parsed keys rather than as strings, so
...
func ResolveWorkItemDir(b *root.Backlog, ref string) (string, error) {
	...
	// A former key resolves here too, or `--work-item` would be the one door a
	// migrated key could not open --- the directory is renamed by the
	// migration, so the old key survives only in the record's former_keys.
	//
