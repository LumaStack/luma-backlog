...
func TestNoCommandWritesOutsideTheBacklog(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		{"task", "new", "Add the queue", "--work-item", "payments-v2"},
		{"decision", "new", "Use a queue", "--work-item", "payments-v2"},
		{"exploration", "new", "Queue options", "--work-item", "payments-v2"},
		{"work-item", "transition", "payments-v2", "in_progress"},
		{"work-item", "journal", "a line worth keeping"},
		{"outcome", "verify", "the-queue-drains", "proven", "-e", "ran it"},
		{"transition", "add-the-queue", "closed"},
