...
func newNewCommand(a *App, unit string) *cobra.Command {
	var workItem, kind, title, description string
	var project bool
	...
	cmd := &cobra.Command{
		...
		Long: "Creates a record from a title. Everything else is derived: the file name\n" +
			...
			"The title is positional or --title. The positional is what a person\n" +
			"types; the flag is what survives being generated by something else.\n\n" +
			"Running it twice with the same title leaves the first one alone.\n\n" +
			longFor(unit),
		Args:         cobra.MaximumNArgs(1),
		SilenceUsage: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			// Both forms are accepted and supplying both is refused, rather
			// than one silently winning. A caller that passed two titles
			// believes something untrue about the record it is about to get,
	...
	cmd.Flags().StringVar(&title, "title", "",
		"the title, for a caller that would rather not pass it positionally")
	cmd.Flags().StringVarP(&description, "description", "d", "",
		"prose about the record, written at creation")
...
func runNew(a *App, cmd *cobra.Command, unit, title, description, workItem, kind string, project, workItemGiven bool) error {
	s, err := open(a)
	if err != nil {
		return err
	}
