...
verify_by:
  ...
  - Close a different deliverable as cancelled with an unmet outcome; expect success, because only delivery is gated.
...
The claim the whole design rests on, and the tool's only refusal (`SPEC.md` §5.0). The cancelled case is part of the outcome rather than a separate one, because gating cancellation would be the obvious wrong implementation.
