...
`[[decisions/the-backlog-unit-is-a-work-item]]` renamed the unit, and everything still says `deliverable`: `spec.md`, the type definitions under `_types/`, the Go implementation, the live corpus in `.backlog/`, and the Claude skills. Until the rename lands everywhere, the vocabulary is split.
